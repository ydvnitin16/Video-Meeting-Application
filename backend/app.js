...
io.on('connection', (socket) => {
    ...
    socket.on('join_room_offer', ({ roomId, offer }) => {
        ...
        socket.to(roomId).emit('join_room_offer', { offer, sender: socket.userId })
    })

    socket.on('join_room_answer', ({ roomId, answer }) => {
        socket.to(roomId).emit('join_room_answer', ({ sender: socket.userId, answer }));
    })

    socket.on('ice-candidate', ({ roomId, candidate }) => {
        socket.to(roomId).emit('ice-candidate', { sender: socket.userId, candidate })
    })
})
...
app.post('/api/v1/participants', async (req, res) => {
    ...
    try {
        const participant = await Participant.create({ roomId, participantId: userId });
        const room = roomMap.get(roomId.toString());
        room.push(userId.toString());
        const existingParticipants = room.filter(r => r.toString() !== userId.toString());
        res.status(201).json({ success: true, message: 'Joined room.', participant, existingParticipants })
    } catch (err) {
        console.log(err);
        res.status(500).json({ success: false, message: err.message })
    }
...
app.post('/api/v1/signup', async (req, res) => {
    ...
    try {
        ...
        if (isExists) {
            res.cookie('user_session', `${isExists._id}`, {
                httpOnly: true,
